...
if(!future.includes(`||'${version}'`)&&!future.includes(`const releaseVersion='${version}'`))throw new Error(`Command Center download version is stale. Expected ${version}.`);
...
for(const required of ['site/atelier.css','site/atelier.js','site/device.css','site/quality.css','site/quality.js']){
...
for(const asset of ['./device.css','./atelier.css','./atelier.js','./quality.css','./quality.js'])if(!sw.includes(`'${asset}'`))throw new Error(`Premium website asset missing from offline shell: ${asset}`);
...
const intelligence=fs.readFileSync(path.join(root,'site/intelligence.js'),'utf8');
if(!intelligence.includes("quality.src='quality.js'"))throw new Error('Final quality-audit layer is not chained into the website.');
console.log(`PASS all website download surfaces match PermissionScope ${version}; monitor-only product render, localization audit and offline cache are wired`);
