...
const report = { passed:true, textScale:2, viewport:{ width:1280, height:800 }, locales, engines:{} };
const errors = [];
...
try {
  for (const [engineName, type] of engines) {
    ...
    try {
      for (const locale of locales) {
        ...
        assert.equal(await page.evaluate(() => getComputedStyle(document.documentElement).fontSize), '32px', `${engineName}/${locale}: 200% root text resize was not applied`);
        await assertNoHorizontalOverflow(page, `${engineName}/${locale}/text-200`);

        const command = page.locator('.ps-command-launch');
        const assistant = page.locator('.ps-assistant-launch');
        assert(await command.isVisible(), `${engineName}/${locale}: command control hidden at 200% text`);
        assert(await assistant.isVisible(), `${engineName}/${locale}: assistant control hidden at 200% text`);
        assert(await page.locator('#download').isVisible(), `${engineName}/${locale}: download section hidden at 200% text`);

        await command.click();
        assert(await page.locator('.ps-command').evaluate(el => el.open), `${engineName}/${locale}: command dialog failed at 200% text`);
        await page.keyboard.press('Escape');
        await assistant.click();
        assert(await page.locator('.ps-assistant').evaluate(el => el.open), `${engineName}/${locale}: assistant dialog failed at 200% text`);
        await page.keyboard.press('Escape');
  ...
  fs.writeFileSync(path.join(root, 'artifacts', 'site-text-resize-compatibility.json'), JSON.stringify(report, null, 2));
  console.log(JSON.stringify(report));
} catch (error) {
  report.passed = false;
  errors.push(error.stack || error.message);
  fs.mkdirSync(path.join(root, 'artifacts'), { recursive:true });
  fs.writeFileSync(path.join(root, 'artifacts', 'site-text-resize-compatibility.json'), JSON.stringify({ ...report, errors }, null, 2));
  throw error;
} finally {
  server.close();
}
